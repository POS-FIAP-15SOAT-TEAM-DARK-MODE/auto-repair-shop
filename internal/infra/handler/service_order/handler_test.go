...
import (
	...
	handler "github.com/POS-FIAP-15SOAT-TEAM-DARK-MODE/auto-repair-shop/internal/infra/handler/service_order"
)
...
func TestGetHistoryByID(t *testing.T) {
	...
	tests := []struct {
		name           string
		serviceID      string
		mockSetup      func(m *mocks.ServiceOrderService)
		expectedCode   int
		expectedItems  int
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			mSvc := mocks.NewServiceOrderService(t)
			if tt.mockSetup != nil {
				tt.mockSetup(mSvc)
			}
