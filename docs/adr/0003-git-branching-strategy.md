...
- Matches the brief's explicit requirement across all 4 repos.
- `develop`→`main` merges are a deliberate, visible checkpoint — appropriate given that merge also triggers a `PRD` deploy.
- Consistent convention across all 4 Tech Challenge repos simplifies onboarding a reviewer who works across more than one of them.
...
- **B. GitFlow with release branches.** More ceremony (separate `release/*` branches) than a small team/coursework project needs; two-stage flow captures the useful part (a review-friendly staging branch) without the extra overhead.
