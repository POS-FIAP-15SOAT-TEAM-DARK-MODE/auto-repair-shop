...
CREATE TABLE customer (
  ...
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
  CHECK (
    (type = 'INDIVIDUAL' AND cpf IS NOT NULL AND cnpj IS NULL AND company_name IS NULL) OR
    (type = 'COMPANY' AND cpf IS NULL AND cnpj IS NOT NULL AND company_name IS NOT NULL)
  )
);
...
CREATE INDEX idx_service_order_service_service_order_id ON service_order_service(service_order_id);
