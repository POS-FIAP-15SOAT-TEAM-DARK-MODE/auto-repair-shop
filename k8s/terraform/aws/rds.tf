...
# in-cluster Postgres in the local kustomize overlay.
resource "random_password" "db" {
  length  = 24
  special = false
}

# JWT signing secret, generated per environment.
...
# Workload secrets in Secrets Manager. The next slice wires External Secrets
# Operator in the cluster to sync this into the app's Kubernetes Secret.
